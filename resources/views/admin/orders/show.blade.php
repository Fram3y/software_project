    ...
    <div class="bg-primary">
        <div class="container col-lg">
            <div>
                ...
                <h1 class="text-light py-2 pb-4">{{ $movie->title }}</h1>
                <h3 class="text-light py-2">Time : {{ $screening->time }}</h3>
                <h3 class="text-light py-2">Cinema : {{ $cinema->name }}</h3>
        ...
        <div class="container">
            <div>
                <form action="{{ route('admin.orders.destroy', $order) }}" method="POST">
                    @method('delete')
                    @csrf
                    <p class="m-0 py-4"><button type="submit" class="btn btn-secondary"
                            onclick="return confirm('Are you sure you wish to cancel your order?')">Cancel Order</button>
                    </p>
                </form>
            </div>
        </div>
